...
export default function SportsPage() {
  ...
  const [activities, setActivities] = useState<ActivityType[]>([
    ...
    },
    {
      ...
      time: "17:00",
      duration: "45 dakika",
      description: "Esneme ve nefes egzersizleri",
      completed: true,
      category: "Esneklik",
      color: "purple"
    },
  ...
  const handleSaveActivity = () => {
    if (!newActivity.title || !newActivity.time || !newActivity.duration) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 px-4 py-8">
      ...
      <div className="relative bg-gradient-blue text-white p-8 rounded-2xl shadow-lg overflow-hidden mb-8 fade-in">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          <div>
            <div className="flex items-center">
              ...
              <h1 className="text-2xl sm:text-3xl font-bold">Spor Aktiviteleri</h1>
            </div>
            <p className="mt-2 text-blue-100 max-w-lg">Sağlıklı bir yaşam için düzenli fiziksel aktivite önemlidir. Koşu, yoga, yüzme gibi aktivitelerinizi takip edin.</p>
          </div>
          <button 
      ...
      <div className="grid gap-4 fade-in" style={{ animationDelay: '0.2s' }}>
        {filteredActivities.length > 0 ? (
          filteredActivities.map((activity) => (
            <div
              key={activity.id}
              className="card p-5 sm:p-6 hover:shadow-md transition-all border-l-4 border-l-blue-500"
            >
                ...
                <div className="flex items-center">
                  <div className="relative">
                    <input
                      ...
                      className="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 peer"
                ...
                <div className="flex-1">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className={`badge ${getCategoryColor(activity.color)}`}>
                        {activity.category}
                      </span>
                    </div>
                  </div>
          ...
          <div className="card p-10 text-center">
            ...
            <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-2">Aktivite bulunamadı</h3>
            <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">Aramanızla eşleşen aktivite yok veya henüz aktivite eklenmemiş. Yeni bir aktivite eklemek ister misiniz?</p>
            <button 
              onClick={handleAddActivity}
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Takım Sporu">Takım Sporu</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-3 mt-1">
                    <button
                      type="button"
                      onClick={() => setNewActivity({...newActivity, color: 'blue'})}
                      className={`w-10 h-10 rounded-full bg-gradient-blue hover:opacity-90 transition ${newActivity.color === 'blue' ? 'ring-2 ring-offset-2 ring-blue-500' : ''}`}
                    />
                    <button
                      type="button"
                      onClick={() => setNewActivity({...newActivity, color: 'purple'})}
                      className={`w-10 h-10 rounded-full bg-gradient-purple hover:opacity-90 transition ${newActivity.color === 'purple' ? 'ring-2 ring-offset-2 ring-purple-500' : ''}`}
                    />
                    <button
                      type="button"
                      onClick={() => setNewActivity({...newActivity, color: 'red'})}
                      className={`w-10 h-10 rounded-full bg-gradient-red hover:opacity-90 transition ${newActivity.color === 'red' ? 'ring-2 ring-offset-2 ring-red-500' : ''}`}
                    />
                    <button
                      type="button"
                      onClick={() => setNewActivity({...newActivity, color: 'green'})}
                      className={`w-10 h-10 rounded-full bg-gradient-green hover:opacity-90 transition ${newActivity.color === 'green' ? 'ring-2 ring-offset-2 ring-green-500' : ''}`}
                    />
                  </div>
                </div>
              ...
              <div className="mt-8 flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 text-sm font-medium text-white bg-gradient-blue border border-transparent rounded-lg hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all"
                >
      ...
      <div className="fixed right-6 bottom-6 z-20">
        <button 
          onClick={handleAddActivity}
          className="btn-primary h-14 w-14 rounded-full flex items-center justify-center shadow-lg hover:shadow-xl"
        >
